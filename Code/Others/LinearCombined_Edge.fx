...
float3 PS_Main(float4 pos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    float DepthDiffEdge = GetDepthDiffEdge(texcoord) * DepthDiffMask; // W3 WeightSilhouette exclusively is applied to DepthDiffEdge

    // Final Edge 

    float finalEdge = WeightSilhouette * DepthDiffEdge;
    ...
    { 
        ...
        finalEdge += adaptive_scalar * WeightStructure * (ScharrEdge + KirschEdge);;
    }
    else 
    {
        finalEdge += WeightStructure * (ScharrEdge + KirschEdge);
    }

    finalEdge += WeightTexture * DoGEdge;
